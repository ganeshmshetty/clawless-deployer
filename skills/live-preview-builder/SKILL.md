...
## Purpose
Generate a self-contained, shareable ClawLess deployment package that lets anyone run the agent in their browser with a single click — no install, no backend, no configuration.
...
1. Write the preview files to `dist/clawless-preview/`:
   - `index.html` — the self-contained preview page
   - `agent-bundle.js` — the FileSystemTree as a JS module
   - `policy.yaml` — the embedded policy
   - `README.md` — instructions for serving locally or deploying
...
## Files Generated
- `dist/clawless-preview/index.html` — Open in browser to run
- `dist/clawless-preview/agent-bundle.js` — Agent filesystem tree
- `dist/clawless-preview/policy.yaml` — Security policy
...
## Guardrails
- Never embed API keys in the generated HTML or bundle — always prompt at runtime.
