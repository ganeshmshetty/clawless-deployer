...
function buildResponseCache(profile) {
  ...
  cache.push({
    ...
    response: `I'm **${profile.name}** (v${profile.version}). ${profile.description}\n\n${profile.soul.identity}\n\n⚠️ *Running in offline mode — responses are cached. Connect to ClawLess for full functionality.*`,
  });
  ...
  cache.push({
    ...
    response:
      ...
      `\n\n⚠️ *Running in offline mode — skill execution requires ClawLess WebContainer.*`,
  });
  ...
  for (const skill of profile.skills) {
    ...
    cache.push({
      ...
      response:
        ...
        `\n\n⚠️ *Offline mode — showing cached skill information only.*`,
    });
  }
  ...
  cache.push({
    ...
    response:
      ...
      `\n\n⚠️ *Offline mode — showing agent rules from configuration.*`,
  });
  ...
  cache.push({
    ...
    response:
      ...
      `3. Click "Run Agent"\n\n` +
      `The agent runs entirely in-browser via WebContainers — no server required.\n\n` +
      `⚠️ *You're currently in offline mode. Reconnect for full functionality.*`,
  });

  return cache;
}

// ── Offline Engine Generator ────────────────────────────────

function generateOfflineEngine() {
  return `/**
 * offline-agent.js — Lightweight offline conversation engine
...
⚠️ *This is an offline fallback response.*\`;
  }
...
<head>
  ...
  <title>${agentName} — Offline Mode</title>
  <style>
    :root {
      --bg: #0a0a0f;
      --card: #1a1a2e;
...
<body>
  ...
  <footer>Offline fallback mode · v${agentVersion} · Interactions are queued for sync</footer>
...
function main() {
  ...
  console.log('');
  console.log('✅ Offline fallback bundle complete!');
  console.log('');
  ...
  if (totalSize > maxSize) {
    console.log('');
    console.warn(`⚠️  Warning: bundle exceeds ${(maxSize / 1024 / 1024).toFixed(0)} MB limit`);
    process.exit(1);
  }
}
