...
// ââ Agent Knowledge Extraction ââââââââââââââââââââââââââââââ
...
// ââ Response Cache Builder ââââââââââââââââââââââââââââââââââ
...
function buildResponseCache(profile) {
  ...
  cache.push({
    ...
    response: `I'm **${profile.name}** (v${profile.version}). ${profile.description}\n\n${profile.soul.identity}\n\nâ ï¸ *Running in offline mode â responses are cached. Connect to ClawLess for full functionality.*`,
  });
  ...
  cache.push({
    ...
    response:
      ...
      `\n\nâ ï¸ *Running in offline mode â skill execution requires ClawLess WebContainer.*`,
  });
  ...
  for (const skill of profile.skills) {
    ...
    cache.push({
      ...
      response:
        ...
        `\n\nâ ï¸ *Offline mode â showing cached skill information only.*`,
    });
  }
  ...
  cache.push({
    ...
    response:
      ...
      `\n\nâ ï¸ *Offline mode â showing agent rules from configuration.*`,
  });
  ...
  cache.push({
    ...
    response:
      ...
      `3. Click "Run Agent"\n\n` +
      `The agent runs entirely in-browser via WebContainers â no server required.\n\n` +
      `â ï¸ *You're currently in offline mode. Reconnect for full functionality.*`,
  });

  return cache;
}

// ââ Offline Engine Generator ââââââââââââââââââââââââââââââââ

function generateOfflineEngine() {
  return `/**
 * offline-agent.js â Lightweight offline conversation engine
...
â ï¸ *This is an offline fallback response.*\`;
  }
...
// ââ Service Worker Generator ââââââââââââââââââââââââââââââââ

function generateServiceWorker() {
  return `/**
 * sw.js â Service Worker for offline fallback
 * Auto-generated by clawless-deployer offline-fallback skill
 */
...
// ââ Offline HTML Page âââââââââââââââââââââââââââââââââââââââ
...
<head>
  ...
  <title>${agentName} â Offline Mode</title>
  <style>
    :root {
      --bg: #0a0a0f;
      --card: #1a1a2e;
...
<body>
  <header>
    <h1>${agentName}</h1>
    <span class="offline-badge">â¡ Offline</span>
  </header>
  <div id="chat"></div>
  <form id="form">
  ...
  <footer>Offline fallback mode Â· v${agentVersion} Â· Interactions are queued for sync</footer>
...
function main() {
  ...
  console.log('');
  console.log('â Offline fallback bundle complete!');
  console.log('');
  ...
  if (totalSize > maxSize) {
    console.log('');
    console.warn(`â ï¸  Warning: bundle exceeds ${(maxSize / 1024 / 1024).toFixed(0)} MB limit`);
    process.exit(1);
  }
}
